...
first — it holds project context, commands, conventions, the task workflow, commit
rules, boundaries, and the load-bearing process rules. This file adds only what is
**specific to Claude Code** (skills, subagents, hooks, plan mode).
...
spec-verifier, 🟡→✅ verify commit, `finish-task.sh` to merge) is defined in
`AGENTS.md`. The named roles map to subagents under `.claude/agents/`
(`task-executor`, `spec-verifier`, `code-reviewer`, `architect`,
...
how a one-time mistake becomes a permanent guard. The file ships with rules covering
parallel-dispatch worktree isolation, the `git checkout -- <path>` hazard,
smoke-test rationalization, dead-code delegates, and a "Common rationalizations"
table.
