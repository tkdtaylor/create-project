...
7. **Close the task** when ready with `scripts/finish-task.sh <NNN> <slug>` (add `--local` to merge without pushing — e.g. to avoid re-triggering CI on every task in a run). It merges `task/NNN-<slug>` into `main`, deletes the branch, removes the worktree, and **verifies all three actually happened**, exiting non-zero if anything is left behind. Prefer it over a bare `git merge`: a forgotten merge or a leftover branch/worktree becomes a hard error instead of silent drift. (On a clean `git merge` the `auto-cleanup-merge.py` hook also cleans up, but `finish-task.sh` is the explicit, checked path.) If it reports a merge conflict, resolve it and re-run.
